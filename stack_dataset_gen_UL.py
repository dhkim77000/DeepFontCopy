...
def data_gen(fonts_path, dataset_path, start, end, length = 5,  is_upper = True):
    ...
    chars = np.concatenate((upper, lower), axis = 0)
    ...
    for font_path in tqdm(font_list):
        ...
        if(check_U_L(font_path)):
            ...
            image_saver(random_text, dir, font_path,font_name, A_test_img, B_test_img, chars)
            
            
def image_saver(text, dir, font_path, font_name,  A_test_img, B_test_img, chars):
...
def train_img_gen_A(train_text, font_path, index):#images of 4 characters which one of them is omitted
    ...
    font = ImageFont.truetype(font = font_path, size = 512)
    out_train =  Image.new('RGB', (64*26*2,64))
    ...
    for i, char in enumerate(lower):
        ...
        train_img = train_img.resize((64,64),Image.ANTIALIAS)
        train_img = augmentation(train_img)
        out_train.paste(train_img, ((i+26)*64,0))
...
def test_img_gen_A(full_text, font_path):#images of all five characters
    ...
    for i, char in enumerate(lower):
        ...
        out_test.paste(test_img, ((i+26)*64,0))

    return out_test


def test_img_gen_B(font_path):#images of all 52 characters
    upper = np.array(list(string.ascii_uppercase))
    lower = np.array(list(string.ascii_lowercase))
    font = ImageFont.truetype(font = font_path, size = 512)
...
fonts_path = "/home/dhkim/data/"
dataset_path = "/home/dhkim/mc-gan-master/datasets/public_web_fonts_Combined/"
